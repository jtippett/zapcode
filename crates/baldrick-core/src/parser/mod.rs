...
pub fn parse(source: &str) -> Result<Program> {
    ...
    if !ret.errors.is_empty() {
        ...
        return Err(BaldrickError::ParseError(msgs.join("\n")));
    }
...
impl<'a> AstLowerer<'a> {
    ...
    fn unsupported(&self, span: oxc_span::Span, desc: &str) -> BaldrickError {
        BaldrickError::UnsupportedSyntax {
    ...
    fn lower_statement(&mut self, stmt: &ast::Statement<'_>) -> Result<Statement> {
        match stmt {
            ...
            ast::Statement::ImportDeclaration(s) => {
                Err(BaldrickError::SandboxViolation(format!(
            ...
            _ => {
                Err(BaldrickError::UnsupportedSyntax {
            ...
            Some(init) => match init {
                ...
                other => {
                    if let Some(expr_ref) = other.as_expression() {
                        ...
                        return Err(BaldrickError::CompileError(
        ...
        match expr {
            ...
            ast::Expression::Identifier(id) => {
                ...
                match name.as_str() {
                    ...
                    "eval" => Err(BaldrickError::SandboxViolation(
                        "eval is forbidden in the sandbox".to_string(),
                    )),
                    "Function" => Err(BaldrickError::SandboxViolation(
                        "Function constructor is forbidden in the sandbox".to_string(),
                    )),
                    "process" => Err(BaldrickError::SandboxViolation(
                        "process is forbidden in the sandbox".to_string(),
                    )),
                    "globalThis" | "global" => Err(BaldrickError::SandboxViolation(
                        "globalThis/global is forbidden in the sandbox".to_string(),
                    )),
                    "require" => Err(BaldrickError::SandboxViolation(
    ...
    fn lower_assignment_target(&mut self, target: &ast::AssignmentTarget<'_>) -> Result<Expr> {
        match target {
            ...
            _ => Err(BaldrickError::CompileError(
